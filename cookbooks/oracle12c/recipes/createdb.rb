...
directory node[:oracle][:rdbms12c][:dbs_root] do
  owner 'oracle'
  group 'oinstall'
  mode '0755'
end
...
template "#{node[:oracle][:rdbms12c][:ora_home]}/assistants/dbca/templates/default_template.dbt" do
  owner 'oracle'
  group 'oinstall'
  mode '0644'
end
...
template "#{node[:oracle][:rdbms12c][:ora_home]}/assistants/dbca/templates/midrange_template.dbt" do
  owner 'oracle'
  group 'oinstall'
  mode '0644'
end

# Iterate over :oracle[:rdbms12c][:dbs]'s keys, Initializing dbca to
# create a database named after the key for each key whose associated
# value is false, and flipping the value afterwards.
# If :oracle[:rdbms12c][:dbs] is empty, we print a warning to STDOUT.
ruby_block "print_empty_db_hash_warning" do
  block do
    Chef::Log.warn(":oracle[:rdbms12c][:dbs] is empty; no database will be created.")
  end
  action :create
  only_if {node[:oracle][:rdbms12c][:dbs].empty?}
end

node[:oracle][:rdbms12c][:dbs].each_key do |db|
  if node[:oracle][:rdbms12c][:dbs][db]
  ...
  ## Create database. 
  if node[:oracle][:rdbms12c][:dbbin_version] == "12c"
    # 12c
    bash "dbca_createdb_#{db}" do
      user "oracle"
      group "oinstall"
      environment (node[:oracle][:rdbms12c][:env])
      code "dbca -silent -createDatabase -emConfiguration DBEXPRESS -templateName #{node[:oracle][:rdbms12c][:db_create_template]} -gdbname #{db} -sid #{db} -sysPassword #{node[:oracle][:rdbms12c][:sys_pw]} -systemPassword #{node[:oracle][:rdbms12c][:system_pw]}"
    end
    ...
    bash "dbca_createdb_#{db}" do
      ...
      group "oinstall"
      environment (node[:oracle][:rdbms12c][:env])
      code "dbca -silent -createDatabase -templateName #{node[:oracle][:rdbms12c][:db_create_template]} -gdbname #{db} -sid #{db} -sysPassword #{node[:oracle][:rdbms12c][:sys_pw]} -systemPassword #{node[:oracle][:rdbms12c][:system_pw]}"
    end

    # Add to listener.ora a stanza describing the new DB.
    ruby_block "append_#{db}_stanza_to_lsnr_conf" do
      block do
        lsnr_conf = "#{node[:oracle][:rdbms12c][:ora_home]}/network/admin/listener.ora"
        sid_desc_body = "(SID_DESC=(GLOBAL_DBNAME=#{db})(ORACLE_HOME=#{node[:oracle][:rdbms12c][:ora_home]})(SID_NAME=#{db})))"
    ...
    # Configure dbcontrol.
    if node[:oracle][:rdbms12c][:dbconsole][:emconfig]
      # Creating em.rsp file for dbcontrol.
      template "#{node[:oracle][:rdbms12c][:ora_home]}/em.rsp" do
        owner 'oracle'
        group 'oinstall'
        mode '0644'
      end
      ...
      bash "prepare_dbsnmp_user_#{db}" do
        ...
        code <<-EOH2
          ...
          ALTER USER DBSNMP IDENTIFIED BY #{node[:oracle][:rdbms12c][:dbsnmp_pw]};
          exit
          EOL1
        EOH2
      end
      ...
      execute "set_sid_to_em-rsp_#{db}" do
        command "sed -i '2s/.*/SID=#{db}/' #{node[:oracle][:rdbms12c][:ora_home]}/em.rsp"
        user 'oracle'
        group 'oinstall'
      end
      # Reloading the listener's configuration.
      execute "reload_listener_to_register_#{db}" do
        command "#{node[:oracle][:rdbms12c][:ora_home]}/bin/lsnrctl reload"
        user 'oracle'
        group 'oinstall'
        environment (node[:oracle][:rdbms12c][:env])
      end
      ...
      execute "conf_dbcontrol_#{db}" do
        command "export ORACLE_HOME=#{node[:oracle][:rdbms12c][:ora_home]}; emca -config dbcontrol db -repos create -respFile #{node[:oracle][:rdbms12c][:ora_home]}/em.rsp"
        user 'oracle'
        group 'oinstall'
        environment (node[:oracle][:rdbms12c][:env])
      end
    ...
    execute "shred_em_rsp_#{db}" do
      command "/usr/bin/shred -zu #{node[:oracle][:rdbms12c][:ora_home]}/em.rsp"
      user 'root'
      group 'root'
    end
  ...
  ruby_block "set_#{db}_install_flag" do
    block do
      node.set[:oracle][:rdbms12c][:dbs][db] = true
    end
    action :create
  end
  ...
  execute "append_#{db}_to_tnsnames.ora" do
    command "echo '#{db} =\n  (DESCRIPTION =\n    (ADDRESS_LIST =\n      (ADDRESS = (PROTOCOL = TCP)(HOST = #{node[:fqdn]})(PORT = 1521))\n    )\n    (CONNECT_DATA =\n      (SERVICE_NAME = #{db})\n    )\n  )\n\n' >> #{node[:oracle][:rdbms12c][:ora_home]}/network/admin/tnsnames.ora"
    not_if "grep #{db} #{node[:oracle][:rdbms12c][:ora_home]}/network/admin/tnsnames.ora > /dev/null 2>&1"
  end
  ...
  directory "#{node[:oracle][:rdbms12c][:dbs_root]}/#{db}/export" do
    owner 'oracle'
    group 'dba'
    mode '0755'
  end
  ...
  bash "block_change_tracking_#{db}" do
    ...
    code <<-EOH3
       ...
       ALTER DATABASE ENABLE BLOCK CHANGE TRACKING USING FILE '#{node[:oracle][:rdbms12c][:dbs_root]}/#{db}/data1/#{db}_block_change_tracking.trk';
       CREATE DIRECTORY "EXPORT" AS '#{node[:oracle][:rdbms12c][:dbs_root]}/#{db}/export';
       exit
       EOL2
    EOH3
  end
  ...
  directory "#{node[:oracle][:rdbms12c][:dbs_root]}/#{db}/backup1" do
    owner 'oracle'
    group 'oinstall'
    mode '0755'
  end
