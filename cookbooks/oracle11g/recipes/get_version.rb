...
ruby_block 'set_rdbms_version_attr' do
  block do
  	patch_info = %x(sudo -u oracle #{node[:oracle][:rdbms11g][:ora_home]}/OPatch/opatch lsinventory -bugs_fixed | grep -E '(#{node[:oracle][:rdbms11g][:latest_patch][:dirname]}) {3}\\1.* DATABASE PATCH SET UPDATE')
    node.set[:oracle][:rdbms11g][:install_info][:patch_nr] = patch_info[/(?!^.+)\b\d+/]
    node.set[:oracle][:rdbms11g][:install_info][:timestamp_str] = patch_info[/[MTWFS][a-z]+ [JFMASOND][a-z]+ \d{2} \d{2}:\d{2}:\d{2} [A-Z]+ \d{4}/]
    node.set[:oracle][:rdbms11g][:install_info][:version_str] = patch_info[/(\d+\.)+\d+/]
  end
  action :nothing
end
