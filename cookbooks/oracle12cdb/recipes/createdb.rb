...
node[:oracle][:rdbms12c][:dbs].each do |db, there|

  if node[:oracle][:rdbms12c][:dbs][there]
...
# Settingi a flag to indicate, that the database has been created.
  ruby_block "set_#{db}_install_flag" do
    block do
      node.set[:oracle][:rdbms12c][:dbs][there] = true
    end
    action :create
  end
...
# end # of create database.
  ...
  bash "block_change_tracking_#{db}" do
    ...
    code <<-EOH3
       ...
       ALTER DATABASE ENABLE BLOCK CHANGE TRACKING USING FILE '#{node[:oracle][:rdbms12c][:dbs11g_root]}/#{db}/data1/#{db}_block_change_tracking.trk';
       CREATE DIRECTORY "EXPORT" AS '#{node[:oracle][:rdbms12c][:dbs11g_root]}/#{db}/export';
       exit
       EOL2
    EOH3
  end
